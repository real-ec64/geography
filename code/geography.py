...
while True:
    ...
    if completed == True:
        ...
        if select == 1:
            if (total * 100 >= player_log['capitals_score'].iloc[0] * player_log['capitals_num'].iloc[0]) | pd.isna(player_log['capitals_score'].iloc[0]):
        ...
        if select == 2:
            if (total * 100 >= player_log['iso2_score'].iloc[0] * player_log['iso2_num'].iloc[0]) | pd.isna(player_log['iso2_score'].iloc[0]):
                    logs_new.loc[logs_new['player'] == name, 'iso2_score'] = perc
                    logs_new.loc[logs_new['player'] == name, 'iso2_num'] = length
                    print(f'New High Score! {perc}% with {length} countries')
        if select == 3:
            if (total * 100 >= player_log['iso3_score'].iloc[0] * player_log['iso3_num'].iloc[0]) | pd.isna(player_log['iso3_score'].iloc[0]):
